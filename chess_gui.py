...
CHESS_PATH = 'Chess'  # path to the chess pieces
...
model = Model(input_shape = (8, 8, 1), output = 1)
model.load_weights('model-103-0.80.hdf5')
...
def get_best_move(board):
    ...
    for i, move in enumerate(possible_moves):
        ...
        board_temp = board.mirror().copy()
        board_temp.push(move)
        X = np.asarray(serialize_game(board_temp))
        X = np.expand_dims(X, axis=0)
        X = np.expand_dims(X, axis=-1)

        predictions.append(model.get_model().predict(X)[0][0])

        print(move, predictions[i])
    print()
